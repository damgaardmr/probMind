...
def Lautum_information_estimate(p_z_1_prior, p_z_2_prior, p_z_2_posterior, z_2_labels, M=1, N=3):
    ...
    for m in range(M):
        ...
        with scope(prefix=str(m)):
            ...
            for n in range(N):
                with scope(prefix=str(n)):
                    obs_dict = {key: p_z_2_prior_trace.nodes[key]["value"] for key in z_2_labels}

                    for obs in obs_dict:
                        obs_dict[obs].requires_grad = True
                    conditional_model = pyro.condition(p_z_2_posterior, data=obs_dict)
                    conditional_model = poutine.scale(conditional_model, 1 / scale)  # scale all sample sites to get prober log_probs in nested inference
                    ...
                    if m == 0 and n == 0:  # determine name for possible outer scopes!
                        posterior_keys = list(name for name in p_z_2_posterior_trace.nodes.keys() if name.endswith(z_2_labels[0]))
                        outerscope = posterior_keys[0].replace(str(m) + "/" + str(n) + "/" + z_2_labels[0], "")
